...
当前代码已实现上述正式状态、受控转换、领域 Command Handler Registry、`Idempotency-Key` 复用和旧 v1 HTTP 缓存确定性迁移。无法安全映射的旧记录进入 `MANUAL_REQUIRED`。
...
4. 当前 `/api/iam/me.factoryIds` 仍包含命令的 `factoryId`。
